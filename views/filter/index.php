      ...
      <?php foreach ($books as $book): ?>
        <article class="book-card" style="position:relative">
          ...
          <div class="book-card-body">
            ...
            <a class="secondary-btn" href="<?= site_url('product/details/'.$book['id']) ?>">View Details <i class="fa-solid fa-arrow-right"></i></a>
          </div>
        </article>
      <?php endforeach; ?>
      </div>
...
<style>.bs-filter-heart{position:absolute;right:12px;top:12px;z-index:5;width:38px;height:38px;border-radius:50%;border:1px solid #e7e9ee;background:#fff;color:#596275;display:grid;place-items:center;cursor:pointer;box-shadow:0 5px 14px rgba(0,0,0,.08)}.bs-filter-heart.is-wishlisted{color:#e4475a;background:#fff5f6;border-color:#f3c8ce}</style>
