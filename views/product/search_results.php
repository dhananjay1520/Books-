                    ...
                    <div class="product-card" data-id="<?php echo htmlspecialchars($product['id']); ?>" style="position:relative"><button class="wishlist-toggle search-wish <?= isset($wishlistStatus[(int)$product['id']]) ? 'is-wishlisted' : ''; ?>" type="button" data-product-id="<?= (int)$product['id']; ?>" aria-label="<?= isset($wishlistStatus[(int)$product['id']]) ? 'Remove from wishlist' : 'Add to wishlist'; ?>" aria-pressed="<?= isset($wishlistStatus[(int)$product['id']]) ? 'true' : 'false'; ?>"><i class="<?= isset($wishlistStatus[(int)$product['id']]) ? 'fa-solid' : 'fa-regular'; ?> fa-heart"></i></button>
                        ...
                        <div class="price-add-container">
                            <?php if (!empty($cartStatus[$product['id']])): ?>
                                <a class="btn-add go-to-cart-btn" href="<?= site_url('cart'); ?>"><i class="fa fa-shopping-bag"></i>&nbsp;Go To Cart</a>
                            <?php else: ?>
    ...
    $('.add-to-cart-form').on('submit', function(event) {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (data.success) {
                    $('#successPopup').fadeIn();
                    form.find('button').replaceWith('<a class="btn-add go-to-cart-btn" href="<?= site_url('cart'); ?>"><i class="fa fa-shopping-bag"></i>&nbsp;Go To Cart</a>');
...
<style>.go-to-cart-btn{display:inline-flex!important;align-items:center;justify-content:center;text-decoration:none!important}.go-to-cart-btn:hover{color:#fff!important;}</style>

<style>.go-to-cart-btn{display:inline-flex!important;align-items:center;justify-content:center;text-decoration:none!important;background:#ea4448!important;color:#fff!important;border-color:#ea4448!important;box-sizing:border-box}.go-to-cart-btn:hover{background:#d93a3e!important;color:#fff!important}</style>

<style>.search-wish{position:absolute;right:10px;top:10px;z-index:4;width:40px;height:40px;border-radius:50%;border:1px solid #e7e9ee;background:#fff;color:#596275;display:grid;place-items:center;cursor:pointer;box-shadow:0 5px 14px rgba(0,0,0,.08)}.search-wish.is-wishlisted{color:#e4475a;background:#fff5f6;border-color:#f3c8ce}</style>
