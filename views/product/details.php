...
.detail-wish{border:1px solid #e2e4eb;background:#fff;border-radius:10px;padding:9px 12px;color:#596275;font-weight:750;cursor:pointer;display:inline-flex;gap:8px;align-items:center}.detail-wish.is-wishlisted{color:#d94255;background:#fff5f6;border-color:#f3c8ce}</style>
...
<div class="product-page-wrapper">
    <div class="product-details-card">
        ...
        <div class="product-info-panel">
            ...
            <div style="display:flex;justify-content:flex-end;margin-bottom:12px"><button type="button" class="detail-wish <?= !empty($isWishlisted) ? "is-wishlisted" : ""; ?>" data-product-id="<?= (int)$product["id"]; ?>"><i class="<?= !empty($isWishlisted) ? "fa-solid" : "fa-regular"; ?> fa-heart"></i><span><?= !empty($isWishlisted) ? "Saved to Wishlist" : "Add to Wishlist"; ?></span></button></div><div class="button-container">
...
const detailWish=document.querySelector('.detail-wish');if(detailWish){detailWish.addEventListener('click',function(){const b=this;fetch('<?= site_url('wishlist/toggle'); ?>',{method:'POST',headers:{'Content-Type':'application/x-www-form-urlencoded; charset=UTF-8'},body:'product_id='+encodeURIComponent('<?= (int)$product['id']; ?>')}).then(r=>r.json()).then(d=>{if(!d.success)throw new Error(d.message||'Wishlist failed');b.classList.toggle('is-wishlisted',!!d.added);const i=b.querySelector('i');if(i){i.classList.toggle('fa-solid',!!d.added);i.classList.toggle('fa-regular',!d.added);}b.querySelector('span').textContent=d.added?'Saved to Wishlist':'Add to Wishlist';document.querySelectorAll('#wishlistCount,.wishlist-count-side').forEach(x=>x.textContent=d.wishlist_count);}).catch(e=>alert(e.message));});}
</script>
